...
<body>
	<%
		...
		if(!userID.equals(daelim.getUserID())){
			...
			if(request.getParameter("daelimTitle") == null || request.getParameter("daelimContent") == null
				|| request.getParameter("daelimTitle").equals("") || request.getParameter("daelimContent").equals("")){
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = daelimDAO.update(daelimID, request.getParameter("daelimTitle"), request.getParameter("daelimContent"));
				// 데이터베이스 오류인 경우
				if(result == -1){
					PrintWriter script = response.getWriter();
					script.println("<script>");
